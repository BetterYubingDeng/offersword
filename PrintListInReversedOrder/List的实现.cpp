...
//typedef struct
//{
//    Excelelem book[100]; //表头信息  char book[100];
//    Liststruct *next;   //
//    Numelem Length;     // int length;
//}ListHead;
//
////******初始化链表******
...
//{
//    Liststruct *Head,*p,*q;
//    Excelelem ch;   //char ch;
//    Head=q=NULL;
//    printf("请输入顺序表Head的内容:\n");
//    while((ch=getchar())!='\n')
//    {
//        p=(Liststruct *)malloc(sizeof(Liststruct));      //对每一个元素重新分配一个ListNode
//        p->book=ch;     //ListNode中的数据域
//        if(!(*i)) Head=q=p,(*i)++;
//        else
//        {
//            q->next=p;        //第一次初始化时候Head = q = p  ,第二次时 q->next指向新分配的ListNode节点
//            q=p;            //再将新节点赋给q作为下一次指向的节点使用
//            (*i)++;
//        }
//    }//注意*q++与(*q)++,有区别！
//    if(q) q->next=NULL;
...
//    Head->Length=0;
//    Head->next=init(&Head->Length);  //作为init(*i)中的i取地址传入，已经被init修改成计数器,记录list的长度
//    return Head;
//}
//
///******打印表中数据内容******/
